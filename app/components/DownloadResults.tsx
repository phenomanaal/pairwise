...
const DownloadResults = ({ onAllDownloadsComplete }: DownloadResultsProps) => {
  ...
  const fetchResults = async () => {
    try {
        ...
        const externalFilesCount = processedResults.filter(r => r.fileType != 'voter').length;


        const alreadyDownloadedCount = processedResults.filter(r => r.downloadingStatus === 'downloaded' && r.fileType != 'voter').length;

        setResults(processedResults);
        setTotalDownloads(externalFilesCount);
        setDownloadedCount(alreadyDownloadedCount);
        setAllDownloadsCompleted(alreadyDownloadedCount === externalFilesCount && externalFilesCount > 0);
        setError(null);
    } catch (err) {
